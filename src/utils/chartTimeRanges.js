...
/**
 * Span threshold above which 60s background polling is wasteful AND
 * dangerous. Set to 7 days.
 *
 * Why this exists:
 *
 *   The May 28, 2026 outage was triggered by sustained long-range chart
 *   queries against `sensor_data`. The cascade was:
 *
 *     1. A chart with a multi-year time range fires its first request.
 *        Backend (pre-hypertable, pre-SQL-aggregation) had to stream
 *        millions of rows through a single async SQLAlchemy session and
 *        bucket them in Python. The query held its Postgres connection
 *        for tens of seconds.
 *     2. SWR's 60s background poll fired before the first query finished.
 *        Now TWO long-running queries are holding connections.
 *     3. Other chart panels on the same page (each component runs two
 *        parallel hooks) compounded the multiplier.
 *     4. `max_connections=100` saturated. Notehub POSTs and worker
 *        registry-cleanup writes started waiting for a free connection.
 *     5. FastAPI workers blocked. Nginx upstream timeouts cascaded.
 *        App "crashed" from the outside even though no process died.
 *
 *   The backend fix (SQL date_bin aggregation + Timescale hypertable)
 *   makes individual queries fast, but the same pattern — long-range
 *   query × parallel hooks × 60s re-poll × multiple users — is still
 *   wasteful work the DB has to do over and over for data that does
 *   not meaningfully change minute-to-minute.
 *
 * Why 7 days:
 *
 *   At ≤7 days the data really IS appending fresh points often enough
 *   that a user staring at the chart can see new readings show up.
 *   Above 7 days, the per-poll deltas are invisible at the chart's
 *   visual resolution — keeping the poll on burns DB cycles for no
 *   user-perceptible benefit.
 *
 *   Also matches the AXIS_FORMATS.DATETIME → AXIS_FORMATS.DATE boundary
 *   (see pickAxisFormatForRange), so the cutoff is consistent with the
 *   point at which the chart itself stops showing time-of-day.
 *
 * Why a `>` comparison (not `>=`):
 *
 *   "Last 7 days" should still poll — it's the boundary case the user
 *   most plausibly leaves open as a dashboard view. The first range
 *   that disables polling is "Last 30 days".
 */
const LONG_RANGE_THRESHOLD_MS = 7 * DAY;

/**
 * Return the appropriate `refreshIntervalMs` to pass to the SWR data
 * hooks (useDeviceMeasurements, useWirelessSensorMeasurements,
 * useDeviceHealth, useMultiWirelessSensorMeasurements).
 *
 * - `0` when the span exceeds LONG_RANGE_THRESHOLD_MS — disables polling
 *   (the hooks honor 0 as "no background refresh"; see the JSDoc at
 *   useDeviceMeasurements:138).
 * - `undefined` for short ranges — the hook's destructuring default
 *   (DEFAULT_REFRESH_INTERVAL_MS, currently 60s) applies. Returning
 *   undefined instead of hard-coding 60000 here means a future change
 *   to the hook's default propagates automatically.
 * - `undefined` for invalid inputs (missing/non-finite dates) so the
 *   caller falls back to default behavior rather than accidentally
 *   disabling polling on a misconfigured component.
 *
 * Usage (in chart panel components):
 *
 *   const refreshIntervalMs = pollingIntervalForRange(from, to);
 *   useDeviceMeasurements(deviceId, { from, to, fields, bucket: 'auto', refreshIntervalMs });
 *
 * @param {Date|string|number|null|undefined} from
 * @param {Date|string|number|null|undefined} to
 * @returns {0|undefined}
 */
export function pollingIntervalForRange(from, to) {
  ...
  // Negative/zero spans aren't a meaningful "long range" — they're a bug
  // in the caller. Fall back to default polling rather than disabling it.
  if (span <= 0) return undefined;
  if (span > LONG_RANGE_THRESHOLD_MS) return 0;
  return undefined;
}
